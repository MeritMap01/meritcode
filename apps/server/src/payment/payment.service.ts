...
export class RazorPayService {
	...
	constructor(private configService: ConfigService, private prisma: PrismaService) {
		...
		if (!keyId || !keySecret) {
			throw new InternalServerErrorException('Razorpay keys are not defined in the environment variables');
		}
	...
	async verifyPayment(paymentData: { razorpay_order_id: string; razorpay_payment_id: string; razorpay_signature: string }) {
		...
		const expectedSignature = this.generateRazorpaySignature(razorpay_order_id, razorpay_payment_id);
	...
	private generateRazorpaySignature(paymentId: string, orderId: string) {
		const hmac = crypto.createHmac('sha256', this.configService.get<string>('RAZORPAY_KEY_ID') as BinaryLike);
